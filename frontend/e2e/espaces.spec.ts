...
test('le libellé et les données changent atomiquement avec X-Mycounts-Espace', async ({
  ...
  await expect(page.getByRole('button', { name: 'Moi' })).toHaveAttribute('aria-current', 'page')

  await page.getByRole('button', { name: 'Maison' }).click()
  // La réponse du nouveau monde est volontairement retardée : l'ancien libellé reste
  // associé à l'ancien contenu, jamais au contenu en cours de chargement.
  await expect(page.getByRole('button', { name: 'Moi' })).toHaveAttribute('aria-current', 'page')
  await expect(page.getByRole('heading', { name: 'Votre premier compte' })).toBeVisible()

  await expect(page.getByRole('button', { name: 'Maison' })).toHaveAttribute(
    'aria-current',
    'page',
  )
...
test('la création ou adhésion tient sans défilement dans 390 x 340', async ({ page }) => {
  ...
  await simulerApi(page)
  await page.goto('/')
  await page.getByRole('button', { name: 'Créer ou rejoindre un foyer' }).click()
  ...
  const cibles = await page
    .locator('nav[aria-label="Changer d’espace"] button, [role="dialog"] button, [role="dialog"] input')
