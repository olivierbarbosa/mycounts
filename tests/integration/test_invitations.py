...
def test_rejoindre_avec_un_code_valide(client: TestClient, session_bd: Session) -> None:
    ...
    reponse = client.post("/auth/rejoindre", json={"code": code, **NOUVEAU})
    assert reponse.status_code == 201
    corps = reponse.json()
    ...
    assert client.get("/auth/moi").status_code == 200, "l'adhésion ouvre directement une session"
...
def test_un_code_ne_sert_qu_une_fois(client: TestClient, session_bd: Session) -> None:
    ...
    assert client.post("/auth/rejoindre", json={"code": code, **NOUVEAU}).status_code == 201
    client.post("/auth/deconnexion")
    seconde = client.post(
        "/auth/rejoindre", json={"code": code, "courriel": "tiers@essai.fr",
...
def test_un_code_expire_est_refuse(client: TestClient, session_bd: Session) -> None:
    ...
    assert client.post("/auth/rejoindre", json={"code": code, **NOUVEAU}).status_code == 403


def test_un_code_inconnu_est_refuse(client: TestClient) -> None:
    reponse = client.post(
        "/auth/rejoindre", json={"code": "code-totalement-invente-mais-long", **NOUVEAU}
    )
