...
def test_un_compte_partage_est_visible_des_deux(session_bd: Session) -> None:
    """Volet inverse : sans lui, une règle qui masquerait TOUT passerait le test
    précédent. Les comptes joints n'arrivent qu'au lot 5, mais la règle de visibilité
    existe déjà et doit être vérifiée dans les deux sens."""
    alice, bruno = foyer_a_deux(session_bd)
    depot.creer_compte(session_bd, alice, nom="Compte joint", prive=False)
    session_bd.commit()

    assert [c.nom for c in depot.comptes_visibles(session_bd, alice)] == ["Compte joint"]
    assert [c.nom for c in depot.comptes_visibles(session_bd, bruno)] == ["Compte joint"]
